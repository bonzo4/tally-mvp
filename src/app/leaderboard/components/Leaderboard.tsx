...
export default function Leaderboard({ title }: { title: string }) {
  return (
    <div className="flex flex-col rounded md:border md:border-gray-200 md:shadow-md space-y-2 lg:space-y-5 p-1 md:p-5">
      <div>
        <h2 className="text-xl lg:text-2xl font-bold">{ title }</h2>
      </div>
      <div className="flex flex-col space-y-3">
        <LeaderboardItem rank={1} name="satoshi nakamoto" value={1000000000}/>
        <LeaderboardItem rank={2} name="i have a really really long name and i don't really know why but it's an edge case" value={218392}/>
